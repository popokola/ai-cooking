...
function VoiceInputTest() {
  ...
  // Handle sending text-based questions
  const handleSendTextQuestion = () => {
    if (textQuestion.trim() === '') {
      // Don't send empty questions
      return;
    }
    addToConversation(textQuestion, 'user');
    setTextQuestion('');
    // You can send this text question to your API in a similar way as voice input
  };

  // Handle sending voice-based questions
  ...
  return (
    <div>
      ...
      <div>
        ...
        />
        <button onClick={handleSendTextQuestion}>Ask</button>
      </div>
